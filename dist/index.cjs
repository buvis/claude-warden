...
function writeScopeDeny(command, cwd, env = process.env) {
  ...
  const { parseCommand: parseCommand2 } = (init_parser2(), __toCommonJS(parser_exports));
  const parsed = parseCommand2(command);
...
function logDecision(config, input, result, elapsedMs, yoloActive) {
  ...
  try {
    ...
    (0, import_fs4.appendFileSync)(config.auditPath, JSON.stringify(entry) + "\n");
  } catch {
  }
}
function rotateIfNeeded(logPath) {
  try {
    const stat = (0, import_fs4.statSync)(logPath);
    if (stat.size >= MAX_LOG_SIZE) {
      (0, import_fs4.renameSync)(logPath, logPath + ".1");
    }
  } catch {
  }
}
...
function yoloFilePath(sessionId) {
  return (0, import_path7.join)((0, import_os7.tmpdir)(), `claude-warden-yolo-${sessionId}`);
}
...
function activateYolo(sessionId, durationMinutes, bypassDeny = false) {
  ...
  (0, import_fs5.writeFileSync)(yoloFilePath(sessionId), JSON.stringify(state), "utf-8");
  return state;
}
...
async function main() {
  ...
  if (typeof command === "string" && command) {
    ...
    if (!scope.armed && scope.disarmed) (0, import_fs6.writeSync)(2, `${DISARM_LINE}
`);
    ...
    if (breach) {
      ...
      process.stdout.write(JSON.stringify(output2));
      (0, import_fs6.writeSync)(2, `${breach}
`);
      process.exit(2);
    }
  }
